...
{
    public:
    void dfs(Node* node, int path[], bool visited[], int pathLen, int k, int& count){
        if (node == NULL)
            return;
        ...
        dfs(node->left, path, visited, pathLen, k, count);
        dfs(node->right, path, visited, pathLen, k, count);
    }
    ...
    int printKDistantfromLeaf(Node* root, int k)
    {
        ...
        dfs(root, path, visited, 0, k, count);
        return count;
    }
};
